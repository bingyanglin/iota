...
use iota_sdk2::types::{Address, Argument, Command};
use iota_types::base_types::ObjectRef;
use iota_types::move_package::MovePackage;
use iota_types::transaction::{CallArg, GasData, ObjectArg, ProgrammableTransaction, TransactionData};
...
pub fn resolve_transaction(
    reader: &GrpcReader,
    unresolved_transaction: &iota_grpc_types::v0::transaction_execution_service::Transaction,
    ...
    let commands = ptb
        .commands
        .iter()
        .map(iota_sdk2::types::Command::try_from)
    ...
    let mut called_packages = called_packages(reader, protocol_config, &commands)?;
    ...
    for move_call in commands.iter().filter_map(|command| {
        ...
        let package = reader
            ...
            .ok_or_else(|| {
                RpcError::new(
                    tonic::Code::NotFound,
                    format!("object {} not found", move_call.package),
                )
            })?
            .data
            .try_as_package()
...
fn resolve_unresolved_transaction(
    ...
    reference_gas_price: u64,
    max_gas_budget: u64,
    sender: iota_sdk2::types::Address,
    unresolved_inputs: &[iota_grpc_types::v0::transaction_execution_service::Input],
    commands: Vec<Command>,
    gas_payment: Option<&iota_grpc_types::v0::transaction_execution_service::GasPayment>,
    expiration: Option<&iota_grpc_types::v0::transaction_execution_service::TransactionExpiration>,
) -> Result<TransactionData> {
    ...
    let expiration = expiration
        .map(iota_types::transaction::TransactionExpiration::try_from)
        .transpose()
        .map_err(|e| {
        ...
        .unwrap_or(iota_types::transaction::TransactionExpiration::None);
...
fn resolve_gas_object_reference(
    ...
    let object = reader
        ...
        .ok_or_else(|| {
            RpcError::new(
                tonic::Code::NotFound,
                format!("object {} not found", unresolved_object_reference.object_id),
            )
        })?;

    let Ok(gas_coin) = iota_types::gas_coin::GasCoin::try_from(&object) else {
...
fn resolve_arg(
    ...
    called_packages: &mut NormalizedPackages,
    commands: &[Command],
    arg: &iota_grpc_types::v0::transaction_execution_service::Input,
    arg_idx: usize,
) -> Result<CallArg> {
    use iota_grpc_types::v0::transaction::input::InputKind;
...
fn resolve_object(
    ...
    arg_idx: usize,
    object_id: Address,
    version: Option<iota_sdk2::types::Version>,
    digest: Option<iota_sdk2::types::Digest>,
    _mutable: Option<bool>,
) -> Result<ObjectArg> {
    let id = object_id.into();
    let object = reader.get_object(&id).ok_or_else(|| {
        RpcError::new(
            tonic::Code::NotFound,
    ...
    match object.owner() {
        ...
        | iota_types::object::Owner::ConsensusAddressOwner { .. } => {
...
pub(super) fn arg_type_of_move_call_input(
    called_packages: &NormalizedPackages,
    move_call: &iota_sdk2::types::MoveCall,
    idx: usize,
...
impl TryFrom<&iota_grpc_types::v0::transaction_execution_service::ObjectReference>
    for UnresolvedObjectReference
{
    type Error = FieldViolation;

    fn try_from(
        value: &iota_grpc_types::v0::transaction_execution_service::ObjectReference,
...
struct UnresolvedInput<'a> {
    pub kind: Option<iota_grpc_types::v0::transaction_execution_service::input::InputKind>,
    pub pure: Option<&'a Bytes>,
    pub object_id: Option<iota_sdk2::types::Address>,
    pub version: Option<iota_sdk2::types::Version>,
    pub digest: Option<iota_sdk2::types::Digest>,
...
impl<'a> UnresolvedInput<'a> {
    fn from_proto(
        input: &'a iota_grpc_types::v0::transaction_execution_service::Input,
    ) -> std::result::Result<Self, FieldViolation> {
        Ok(Self {
