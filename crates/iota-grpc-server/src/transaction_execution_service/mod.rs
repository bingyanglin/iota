...
pub async fn execute_transaction(
    ...
    let include_input_objects = tx_mask
        .as_ref()
        .map(|m| {
            m.contains(ExecutedTransaction::INPUT_OBJECTS_FIELD.name)
                || m.contains(ExecutedTransaction::EFFECTS_FIELD.name)
        })
        .unwrap_or(false);
    let include_output_objects = tx_mask
        .as_ref()
        .map(|m| {
            m.contains(ExecutedTransaction::OUTPUT_OBJECTS_FIELD.name)
                || m.contains(ExecutedTransaction::EFFECTS_FIELD.name)
        })
        .unwrap_or(false);
