...
pub trait ExecutionCacheCommit: Send + Sync {
    ...
    ///
    /// `checkpoint_seq` is the sequence number of the checkpoint that contains
    /// these transactions; it is stamped onto each newly written object's
    /// `previous_transaction_checkpoint` field.
    fn build_db_batch(
        &self,
        epoch: EpochId,
