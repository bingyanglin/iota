...
impl CheckpointReader {
    ...
    async fn sync(&mut self) -> IngestionResult<()> {
        // const CATCH_UP_GAP_MULTIPLIER: u64 = 1; // Times batch_size
        // This const was unused after the threshold was fixed to 50u64 for testing.

        if self.options.use_grpc_streaming {
            if let Some(ref grpc_addr_str) = self.options.grpc_address {
                match Self::get_latest_checkpoint_sequence_number_from_grpc(grpc_addr_str).await {
                    Ok(Some(latest_known_on_node)) => {
                        // let catch_up_threshold =
                        //     self.options.batch_size as u64 * CATCH_UP_GAP_MULTIPLIER;
                        // Use a smaller threshold to trigger catch-up more easily for testing
                        let catch_up_threshold = 1u64; // Changed from 50 to 1 for ultra-quick testing
                        if self.current_checkpoint_number < latest_known_on_node
                            && latest_known_on_node - self.current_checkpoint_number
                                >= catch_up_threshold
                        {
                            info!(
                                "CheckpointReader::sync: Entering gRPC unary catch-up mode. Current: {}, Node latest: {}, Threshold: {}.",
                                self.current_checkpoint_number,
                                latest_known_on_node,
                                catch_up_threshold
                            );

                            // Create a temporary Grpc client for unary calls
                            let grpc_uri = format!("http://{}", grpc_addr_str);
                            match CheckpointGprcServiceClient::connect(grpc_uri.clone()).await {
                                Ok(grpc_unary_client) => {
                                    let mut unary_store = RemoteStore::Grpc(grpc_unary_client);
                                    while self.current_checkpoint_number <= latest_known_on_node {
                                        if self.exceeds_capacity(self.current_checkpoint_number) {
                                            info!(
                                                "CheckpointReader::sync: Catch-up capacity exceeded. Will resume later."
                                            );
                                            break;
                                        }
                                        match Self::remote_fetch_checkpoint(
                                            &mut unary_store,
                                            self.current_checkpoint_number,
                                        )
                                        .await
                                        {
                                            Ok((checkpoint_arc, _size)) => {
                                                info!(
                                                    "CheckpointReader::sync (Catch-up): Fetched CP {} via unary gRPC.",
                                                    checkpoint_arc
                                                        .checkpoint_summary
                                                        .sequence_number
                                                );
                                                // Sequence number should match, as we requested it
                                                // specifically.
                                                // If not, it is an unexpected behavior from
                                                // remote_fetch_checkpoint or the server.
                                                if checkpoint_arc.checkpoint_summary.sequence_number
                                                    != self.current_checkpoint_number
                                                {
                                                    error!(
                                                        "CheckpointReader::sync (Catch-up): Fetched CP {} but expected {}. This is unexpected for unary fetch.",
                                                        checkpoint_arc
                                                            .checkpoint_summary
                                                            .sequence_number,
                                                        self.current_checkpoint_number
                                                    ...
                                                    // could break or attempt to
                                                    // use the given
                                                    // checkpoint's number.
                                                    // For now, we will proceed
                                                    // to send it, and the main
                                                    // sync loop's logic for
                                                    // handling out-of-order CPs
                                                    // might apply,
                                                ...
                                                if let Err(e) = self
                                                    .checkpoint_sender
                                                    .send(checkpoint_arc.clone())
                                                    .await
                                                {
                                                    error!(
                                                        "CheckpointReader::sync (Catch-up): FAILED to send CP {} to executor: {:?}",
                                                        checkpoint_arc
                                                            .checkpoint_summary
                                                            .sequence_number,
                                                        e
                                                    );
                                                    return Err(IngestionError::Channel("unable to send checkpoint to executor during catch-up".to_owned()));
                                                }
                                                info!(
                                                    "CheckpointReader::sync (Catch-up): Successfully sent CP {} to executor. Updated CPNUM to: {}",
                                                    checkpoint_arc
                                                        .checkpoint_summary
                                                        .sequence_number,
                                                    checkpoint_arc
                                                ...
                                                self.current_checkpoint_number = checkpoint_arc
                                                    .checkpoint_summary
                                                    .sequence_number
                                                    + 1;
                                            }
                                            Err(e) => {
                                                error!(
                                                    "CheckpointReader::sync (Catch-up): Failed to fetch CP {} via unary gRPC: {:?}. Pausing catch-up for this cycle.",
                                                    self.current_checkpoint_number, e
                                                );
                                                break;
                                            }
                                        }
                                    ...
                                    info!(
                                        "CheckpointReader::sync: Exiting gRPC unary catch-up mode. Current CP: {}",
                                        self.current_checkpoint_number
                                    );
                                    if self.remote_fetcher_receiver.is_some() {
                                        info!(
                                            "CheckpointReader::sync: Resetting remote_fetcher_receiver after catch-up to re-initiate stream."
                                        );
                                        self.remote_fetcher_receiver = None;
                                    }
                                }
                                Err(e) => {
                                    error!(
                                        "CheckpointReader::sync: Failed to create gRPC client for catch-up to {}: {}. Skipping catch-up for this cycle.",
                                        grpc_uri, e
                                    );
                                }
                            }
                            ...
                            info!(
                                "CheckpointReader::sync: gRPC catch-up not needed. Current: {}, Node latest: {}, Threshold: {}. Proceeding to normal fetch.",
                                self.current_checkpoint_number,
                                latest_known_on_node,
                                catch_up_threshold
                            );
                    ...
                    Ok(None) => {
                        info!(
                            "CheckpointReader::sync: No checkpoints reported by gRPC node (ListCheckpoints returned empty). Proceeding without catch-up."
                        );
                    }
                    Err(e) => {
                        error!(
                            "CheckpointReader::sync: Could not get latest checkpoint from gRPC node for catch-up check: {:?}. Proceeding without catch-up.",
                            e
                        );
                    }
                }
                ...
                info!(
                    "CheckpointReader::sync: gRPC address not configured, skipping gRPC catch-up logic."
                );
            }
        } else {
            info!(
                "CheckpointReader::sync: gRPC streaming not enabled, skipping gRPC catch-up logic."
            );
        }
    ...
    async fn get_latest_checkpoint_sequence_number_from_grpc(
        grpc_address: &str,
    ) -> IngestionResult<Option<CheckpointSequenceNumber>> {
        let grpc_uri = format!("http://{}", grpc_address);
        let mut client = CheckpointGprcServiceClient::connect(grpc_uri.clone())
            .await
            .map_err(|e| {
                error!(
                    "Failed to connect to gRPC endpoint {} to get latest checkpoint: {}",
                    grpc_uri, e
                );
                IngestionError::Upstream(anyhow::anyhow!(
                    "Failed to connect to gRPC endpoint {} for latest checkpoint: {}",
                    grpc_uri,
                    e
                ))
            })?;
        ...
        let request = tonic::Request::new(ListCheckpointsRequest {
            limit: Some(1),
            start_sequence_number: Some(CheckpointSequenceNumber::MAX.to_string()),
            end_sequence_number: None,
            direction: Some(Direction::Descending as i32),
        });

        match client.list_checkpoints(request).await {
            Ok(response) => {
                if let Some(checkpoint_summary_grpc) = response.into_inner().checkpoints.first() {
                    Ok(Some(checkpoint_summary_grpc.sequence_number))
                } else {
                    Ok(None) // No checkpoints found
                }
            }
            Err(status) => {
                error!("gRPC ListCheckpoints call failed: {}", status);
                Err(IngestionError::Upstream(anyhow::anyhow!(
                    "gRPC ListCheckpoints call failed: {}",
                    status
                )))
            }
        }
