...
pub(crate) async fn start_tx_checkpoint_commit_task_v2<S>(
    state: S,
    metrics: IndexerMetrics,
    tx_indexing_receiver: iota_metrics::metered_channel::Receiver<CheckpointDataToCommitV2>,
    mut next_checkpoint_sequence_number: CheckpointSequenceNumber,
    cancel: CancellationToken,
) -> IndexerResult<()>
where
    S: IndexerStoreExt + Clone + Sync + Send + 'static,
{
    use futures::StreamExt;
    ...
    while let Some(indexed_checkpoint_batch) = stream.next().await {
        ...
        while let Some(checkpoint) = unprocessed.remove(&next_checkpoint_sequence_number) {
            ...
            next_checkpoint_sequence_number += 1;
            if batch.len() == checkpoint_commit_batch_size || epoch.is_some() {
                commit_checkpoints_v2(&state, batch, epoch, &metrics).await;
                batch = vec![];
            }
        }
        if !batch.is_empty() && unprocessed.is_empty() {
            commit_checkpoints_v2(&state, batch, None, &metrics).await;
            batch = vec![];
        }
    }
    Ok(())
...
async fn commit_checkpoints_v2<S>(
    state: &S,
    indexed_checkpoint_batch: Vec<CheckpointDataToCommitV2>,
    epoch: Option<EpochToCommit>,
    metrics: &IndexerMetrics,
) where
    S: IndexerStoreExt + Clone + Sync + Send + 'static,
