...
// Auto-completions for name chains, such as `mod::struct::field` or
// `mod::function`, both in the code (e.g., types) and in `use` statements.
//
// Auto-imports for name chains, but only for the first or second component
// where it makes the most sense. In other words:
...
/// Handle name chain auto-completion at a given position. The gist of this
/// approach is to first identify what the first component of the access chain
/// represents (as it may be a package, module or a member) and if the chain has
/// other components, recursively process them in turn to either
...
pub fn name_chain_completions(
    ...
    // there may be access chains for which there is not auto-completion info
    // generated by the compiler but which still have to be handled (e.g.,
    // chains starting with numeric address)
    let info = symbols
        .compiler_info
    ...
    // if we are auto-completing for an access chain, there is no need to include
    // default completions
    ...
    if leading_name.loc.contains(&cursor.loc) {
        // at first position of the chain suggest all packages that are available
        // regardless of what the leading name represents, as a package always
        // fits at that position, for example: OxCAFE::...
        // some_name::...
        // ::some_name
        //
        completions.extend(
        ...
        // only if leading name is actually a name, modules or module members are a
        // correct auto-completion in the first position, and auto-imports may
        // kick in
...
/// Handles auto-completions for "regular" `use` declarations (name access
/// chains in `use fun` declarations are handled as part of name chain
/// completions).
pub fn use_decl_completions(
    ...
    // if we are auto-completing for a use decl, there is no need to include default
    // completions
    completion_finalized = true;

    // there is no auto-completion info generated by the compiler for this but
    // helper methods used here are shared with name chain completion where it
    // may exist, so we create an "empty" one here
...
/// Handles auto-completion for structs and enums variants, including fields
/// contained by the struct or variant.
fn datatype_completion(
    ...
    // only add fields if there are some and we are in the same module as the
    // datatype
...
/// Returns all possible completions for a module member (e.g., a datatype)
/// component of a name access chain, where the prefix of this component (e.g,
/// in `some_pkg::some_mod::`) represents a module specified in
/// `prefix_mod_ident`. The `inside_use` parameter determines if completion is
...
/// Returns completion item if a given name/alias identifies a valid member of a
/// given module available in the completion scope as if it was a single-length
/// name chain. If source module is specified, and it is different from the
/// module of the member, then only public members are included in the
/// completion, unless modules are in the same package, in which case package
/// members are also included. No members are included if a circular dependency
/// is detected. This exclusion is needed to generate correct copletions for
/// auto-imports.
...
/// Returns completion items for all members of a given module as if they were
/// single-length name chains.
...
/// Checks if a given module identifier represents a module in a package
/// identifier by `leading_name`.
...
/// Computes completions for a given chain entry: `prev_kind` determines the
/// kind of previous chain component, and `chain_kind` contains information
/// about the entity that the whole chain may represent (e.g., a type of or a
/// function).
...
/// Computes the kind of the next chain component (based on what the previous
/// one, represented by `prev_kind` was).
...
/// Walks down a name chain, looking for the relevant portion that contains the
/// cursor. When it finds, it calls to `name_chain_entry_completions` to compute
/// and return the completions.
...
fn cursor_at_colon_colon(
    ...
    if path_index == path_entries.len() {
        // the only reason we would not return here is if we were at `::` which is past
        // the location of the last path component
...
/// Get all packages that could be a target of auto-completion, whether they are
/// part of `AliasAutocompleteInfo` or not.
