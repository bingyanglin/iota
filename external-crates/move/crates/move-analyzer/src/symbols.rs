...
pub fn mod_ident_to_ide_string(
    ...
    match mod_ident.address {
        A::Numerical { name, value, .. } => {
            ...
            // check for IOTA implicits only if the previous call determined
                            ...
                            match get_symbols(
                                ...
                                Ok((symbols_opt, lsp_diagnostics)) => {
                                    ...
                                    if let Some(new_symbols) = symbols_opt {
                                        ...
                                        //
                                        // TODO: we may consider "unloading" symbolication
                                        // information when
                                        // files/directories are being closed but as with other
                                        // performance
